...
`<head>` 안의 인라인 스크립트는 `is:inline` 디렉티브로 번들링 없이 HTML에 그대로 출력됩니다.
브라우저가 첫 페인트를 시작하기 전에 `localStorage`와 시스템 테마를 확인해 `.dark` 클래스를 적용하므로, 새로고침 시 흰색 화면이 깜빡이는 FOUC 현상을 방지할 수 있습니다.

> SEO 최적화 구현 상세 내용은 다음 글에서 자세히 다룰 예정입니다.
