...
`client:load`는 페이지 로드 즉시 hydrate하고, `client:visible`은 뷰포트에 진입할 때 hydrate합니다.
...
| nanostores      | 경량 전역 상태 관리 (다크모드)                      |
| Shiki           | 코드 블럭 듀얼 테마 하이라이팅                      |
| Vercel          | 배포 플랫폼                                         |
...
`getStaticPaths()`가 빌드 타임에 모든 글의 경로를 생성하고, 각 경로의 데이터를 `Astro.props`로 전달합니다.
...
> Astro <Tooltip note="이전에는 모든 글 데이터 객체에 render 함수가 포함되어 있어 메모리를 많이 잡아먹었습니다. 이제는 데이터만 가볍게 들고 있다가 필요할 때만 render 함수를 쓰므로 빌드 속도가 훨씬 빨라졌습니다.">v5</Tooltip>에서 `render()`는 Content Layer API의 독립 함수입니다.
