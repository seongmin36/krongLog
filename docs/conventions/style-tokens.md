...
| `--color-primary-surface-dark` | `rgba(240,168,184,0.12)` | 활성 항목 배경(다크)          |

### 검색바

| 토큰                               | 값                      | 용도                                                                |
| ---------------------------------- | ----------------------- | ------------------------------------------------------------------- |
| `--color-search-surface`           | `#fafafa`               | 검색바 배경(라이트)                                                 |
| `--color-search-surface-dark`      | `#2b2c2e`               | 검색바 배경(다크)                                                   |
| `--color-search-border`            | `#ededf0`               | 검색바 테두리·⌘K 배지 배경(라이트)                                  |
| `--color-search-border-dark`       | `#3b3e44`               | 검색바 테두리·⌘K 배지 배경(다크)                                    |
| `--color-search-text`              | `#9aa2ad`               | 검색 아이콘·placeholder·⌘K 텍스트(라이트)                           |
| `--color-search-text-dark`         | `#9ca3af`               | 동일(다크, 가독성을 위해 라이트 원안(`#717a86`)보다 밝게 조정)      |
| `--color-search-inset-shadow`      | `rgba(0,0,0,0.06)`      | 검색바 기본 상태 inset shadow(라이트)                               |
| `--color-search-inset-shadow-dark` | `rgba(0,0,0,0.18)`      | 동일(다크)                                                          |
| `--color-search-glow`              | `rgba(59,122,87,0.2)`   | 검색바 활성화 시 바깥쪽 glow(라이트, `--color-primary`와 동일 색상) |
| `--color-search-glow-dark`         | `rgba(240,168,184,0.2)` | 동일(다크, `--color-dark-text`와 동일 색상)                         |
